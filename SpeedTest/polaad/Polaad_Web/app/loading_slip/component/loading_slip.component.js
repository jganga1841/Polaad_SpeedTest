...
var LoadingSlipComponent = (function () {
    function LoadingSlipComponent(ss, commonServices, bookingServices, loadingServices, _confirmService, builder, _sanitizer, authLoginServices) {
        ...
        this.selectedMaterialLayerDesc = "Bottom";
        ...
        this.isDispLoadingSlipSummery = false;
        this.dealerTo = {};
    ...
    LoadingSlipComponent.prototype.valuechange = function (keyVal, currentBookingInfo) {
        ...
        this.loadingSlipToList.filter(function (p) { return p.LoadingLayerId == _this.selectedMaterialLayerId; }).forEach(function (element) {
            if (element.TblLoadingSlipDtlTO.IdBooking == currentBookingInfo.IdBooking) {
                ...
                element.CdStructure = _this.cdStructureLst.filter(function (p) { return p.Value == currentBookingInfo.CdStructureId; })[0].Text;
    ...
    LoadingSlipComponent.prototype.mapLayerDeliveryAddr = function (layerId, layerDesc) {
        var _this = this;
    ...
    LoadingSlipComponent.prototype.getConfirmNnConfList = function () {
    ...
    LoadingSlipComponent.prototype.getSpecsList = function () {
        var _this = this;
        this.commonServices.getMaterialSepc().subscribe(function (data) {
            _this.productSpecsList = data;
            _this.ss.hideLoader();
        }, function (error) {
            _this.ss.hideLoader();
            console.log(error);
    ...
    LoadingSlipComponent.prototype.CollectMatvsBookedOrder = function (proCategoryVal, dealerBookingInfo) {
        ...
        if (matCatvsSpecBookedList.length == 0 || matCatvsSpecBookedList == undefined) {
            this.loadingServices.getProductStockList(this.loadingSlipTO, this.productCategoryId, myGlobalVal.ProductSpecsIdE.BEND).subscribe(function (data) {
                _this.commonLoadingSlipExtToList = data;
                //this.generateListSizevsSpecs();
                _this.pushAvlQuotaToSizesSelection();
    ...
    LoadingSlipComponent.prototype.pushAvlQuotaToSizesSelection = function () {
        var _this = this;
        this.loadingServices.getCnfDeclaredLoadingQuota(this.selectedCnfId).subscribe(function (data) {
            ...
            if (_this.loadingQuotaAvailableStock.length > 0) {
                _this.calculateAndSetAvlLoadingQuota();
                _this.calculateAndSetAvlQuotaForLoadingSlip();
            }
        });
    };
    LoadingSlipComponent.prototype.calculateAndSetAvlLoadingQuota = function () {
        var _this = this;
        this.commonLoadingSlipExtToList.forEach(function (element) {
            _this.loadingQuotaAvailableStock.filter(function (p) { return p.MaterialId == element.MaterialId
                && p.ProdCatId == element.ProdCatId && p.ProdSpecId == element.ProdSpecId; }).forEach(function (ele) { element.AvlQuota = ele.BalanceQuota + -element.LoadingQty; });
        });
    };
    LoadingSlipComponent.prototype.calculateAndSetAvlQuotaForLoadingSlip = function () {
        var _this = this;
        debugger;
        this.commonLoadingSlipExtToList.forEach(function (element) {
            _this.loadingQuotaAvailableStock.filter(function (p) { return p.MaterialId == element.MaterialId
                && p.ProdCatId == element.ProdCatId && p.ProdSpecId == element.ProdSpecId; }).forEach(function (ele) {
                element.AvlQuota = ele.BalanceQuota;
                _this.loadingSlipToList.forEach(function (p) {
                    p.LoadingSlipExtTOList.filter(function (q) { return q.MaterialId == element.MaterialId
                        && q.ProdCatId == element.ProdCatId && q.ProdSpecId == element.ProdSpecId; }).forEach(function (ele1) { element.AvlQuota += -ele1.LoadingQty; });
                });
            });
        });
    };
    ...
    LoadingSlipComponent.prototype.addLoadingQtyInExtTOList = function () {
        ...
        // })
        this.commonLoadingSlipExtToList.forEach(function (ele) {
            ele.LoadingLayerid = _this.selectedMaterialLayerId;
            ele.LoadingLayerE = _this.selectedMaterialLayerId;
            ele.LoadingLayerDesc = _this.selectedMaterialLayerDesc;
        });
    ...
    LoadingSlipComponent.prototype.calculateTotalEnterQty = function () {
        ...
        this.totalLoadedQty = 0;
        this.commonLoadingSlipExtToList.forEach(function (element) {
            _this.totalLoadedQty += +element.LoadingQty;
        });
    ...
    LoadingSlipComponent.prototype.calculateTotalEnterQty_old = function () {
        var _this = this;
    ...
    LoadingSlipComponent.prototype.addOneMoreSizeInSelection = function (newLoadQtyInExtTo) {
        ...
        if (this.lblErrorMsg == undefined) {
            newLoadQtyInExtTo.MaterialDesc = this.materialList.filter(function (p) { return p.Value == newLoadQtyInExtTo.MaterialId; })[0].Text;
            newLoadQtyInExtTo.ProdCatDesc = this.productCategoryList.filter(function (p) { return p.Value == newLoadQtyInExtTo.ProdCatId; })[0].Text;
            newLoadQtyInExtTo.ProdSpecDesc = this.productSpecsList.filter(function (p) { return p.Value == newLoadQtyInExtTo.ProdSpecId; })[0].Text;
            var vObj = new loading_information_1.LoadingSlipExtTO();
    ...
    LoadingSlipComponent.prototype.validateLoadingAQtyBeforeAdd = function (newLoadQtyInExtTo) {
        debugger;
        var vRes = this.commonLoadingSlipExtToList.filter(function (p) { return p.MaterialId == newLoadQtyInExtTo.MaterialId
        ...
        if (vRes != undefined && vRes.length > 0) {
            ...
            this.lblErrorMsg = undefined;
            var avlStockForSelected = this.loadingQuotaAvailableStock.filter(function (p) { return p.MaterialId == newLoadQtyInExtTo.MaterialId
                && p.ProdCatId == newLoadQtyInExtTo.ProdCatId && p.ProdSpecId == newLoadQtyInExtTo.ProdSpecId; });
    ...
    LoadingSlipComponent.prototype.DisplayOneMoreSizeSelection = function () {
        this.addOneMoreSizeSelection = true;
    };
    LoadingSlipComponent.prototype.generateLoadingSlipsummery = function () {
        debugger;
        this.calculateTotalQtyLayerWise(1);
        this.mergeLoadingSlipAgainestidBooking();
        this.addSelectedLayerLoadingSlipTO();
    ...
    LoadingSlipComponent.prototype.getVehicleNoList = function () {
        var _this = this;
        debugger;
        if (localStorage.getItem("VehicleList") == undefined) {
            this.loadingServices.getVehicleNoList().subscribe(function (p) {
                _this.vehicleToList = p;
                localStorage.setItem("VehicleList", JSON.stringify(_this.vehicleToList));
            });
        }
        else {
        ...
        if (this.vehicleToList.length > 0 && this.vehicleToList != undefined) {
            ...
            this.VehicleNoList = [];
            var i_2 = 0;
            this.vehicleToList.forEach(function (element) {
                debugger;
                i_2 += 1;
                _this.stateCodeList.push({ Value: i_2, Text: element.StateCode });
                _this.DistCodeList.push({ Value: i_2, Text: element.DistrictCode });
                _this.UniqueLetterList.push({ Value: i_2, Text: element.UniqueLetters });
                _this.VehicleNoList.push({ Value: i_2, Text: element.VehicleNo });
            });
        }
    };
    LoadingSlipComponent.prototype.SetFocusFor = function (codeVal) {
        switch (codeVal) {
            case 'StateCode':
    ...
    LoadingSlipComponent.prototype.focusToElement = function (inputName) {
        // this.inputs.toArray().find((e) => {
        // return e.nativeElement.getAttribute('name') == inputName;
        // }).nativeElement.focus();
    };
    LoadingSlipComponent.prototype.allowDigitsValidation = function () {
        debugger;
    ...
    LoadingSlipComponent.prototype.getValidationColor = function (valueOfSizes) {
    ...
    LoadingSlipComponent.prototype.CheckLoadingLayerExist = function (loadingSeqNum) {
        debugger;
        var vRes = this.loadingSlipToList.filter(function (p) { return p.LoadingLayerId == loadingSeqNum; });
        if (vRes == undefined || vRes.length == 0) {
            return false;
        }
        else {
    ...
    LoadingSlipComponent.prototype.validateLoadingSlipLoadedAgnLayer = function () {
        debugger;
