...
var AgentsServices = (function () {
    ...
    AgentsServices.prototype.getDealerAddrInfo = function (dealerId) {
        ...
        return this.http.get(this.CommonUrl + 'Address/GetOrgAddressDetails?orgId=' + dealerId)
    ...
    AgentsServices.prototype.getLastFourBookingHist = function (dealerId, lastNRecords) {
        ...
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        return this.http.get(this.CommonUrl + 'Booking/GetDealerBookingHistory?dealerId=' + dealerId + '&lastNRecords=' + lastNRecords)
            .map(function (res) { return res.json(); })
            .catch(function (error) { return Rx_1.Observable.throw(error.json().error || 'Server error'); });
